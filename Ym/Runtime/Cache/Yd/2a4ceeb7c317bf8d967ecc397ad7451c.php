...
<title><?php echo get_config_value('site_name');?> - 增加数字营销</title>
	 ...
	 <div class="logo"><img  src="<?php echo get_config_value('site_logo');?>" /></div>
              ...
              <i class="icon-off icon-white"></i> <a id="modal-973558" href="#modal-container-973558" role="button" data-toggle="modal">退出</a> <i class="icon-user icon-white"></i> <a href="/" target="_blank">网站首页</a>
				...
				<div class="modal-footer">
					 <button class="btn" data-dismiss="modal" aria-hidden="true">关闭</button> <a class="btn btn-primary" style="line-height:20px;" href="/Yd/Sz/logout/" >确定退出</a>
				</div>
			</div>
				</div>
</div>
...
<ul class="breadcrumb">当前位置： 后台管理
   ...
   <div class="title_right"><strong><?php if($goods_info): ?>编辑<?php else: ?>添加<?php endif; ?>数字营销</strong></div>
   <div style="width:650px;">
   	<form  action="<?php echo U('/Yd/Sz/save');?>" method="post" enctype="multipart/form-data">
      <input name="id" type="hidden" value="<?php echo ($goods_info["id"]); ?>" />
      <input name="w7" type="hidden" value="3" />
   ...
   <tr>
     ...
     <tr>
       <td align="right" nowrap="nowrap" bgcolor="#f1f1f1">名称：</td>
       ...
       <tr>
           <td align="right" nowrap="nowrap" bgcolor="#f1f1f1">图片：</td>
           <td width="68%">
               <input  name="z1" type="file" value="">
               <span class="Validform_checktip">尺寸：900 X 80</span></td>
       </tr>
   </table>
